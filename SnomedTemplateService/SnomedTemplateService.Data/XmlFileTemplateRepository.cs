...
namespace SnomedTemplateService.Data
{
    public class XmlFileTemplateRepository : ITemplateRepository
    {
        ...
        public TemplateData GetById(int id)
        {
            ...
            {
                return ExtractTemplateDataFromNode(templateNode);
            }
        ...
        {
            return sourceDoc.SelectNodes($"//template")
                .Cast<XmlNode>()
                .Select(ExtractTemplateDataFromNode)
                .ToList();
        }

        private TemplateData ExtractTemplateDataFromNode(XmlNode templateNode)
        {
            if (int.TryParse(templateNode.Attributes["id"]?.Value, out int id))
            {
